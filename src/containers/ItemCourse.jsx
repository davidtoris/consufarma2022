...
const ItemCourse = ({ curso }) => {
  ...
  const [courseDetails, setCourseDetails] = useState(curso);
  const [isValidating, setIsValidating] = useState(true);

  useEffect(() => {
    const fetchFreshData = async () => {
      if (!curso.nombre_ruta) return;
      try {
        // Fetch fresh data to validate dates
        const res = await axios.get(`${API_BASE_URL}/courses/${curso.nombre_ruta}`);
        if (res.data && res.data.course) {
          const { fecha, fecha_text, label } = res.data.course;
  ...
  const { id, nombre, nombre_ruta, fecha, fecha_text, duracion, horario, especialidad_id, ponente_uno_id, ponente_dos_id, objetivo, temario, precio, precioUSD, descuento, imagen, register, lugar, disponible, label } = courseDetails;
  ...
  const especialidades = async () => {
    const res = await axios.get(`${API_BASE_URL}/courses/especialidad/${especialidad_id.id}`);
  ...
  useEffect(() => {
    ...
    const itemIndex = lastItems.findIndex((item) => item.id === id);
    if (itemIndex === -1) {
      // const updatedItems = ;
        ...
        <div className='col-span-1 md:col-span-2 md:px-0 px-3'>
          ...
          <div className='date-hour-course bg-blueConsufarma p-2 text-white rounded-md flex md:w-[40rem] w-100 flex-col md:flex-row font-bold text-lg mt-3'>
            ...
            <div className='flex ml-1 md:ml-4 items-center'>
              ...
              {isValidating ? (
                <span className="text-sm italic ml-2">Validando fecha...</span>
              ) : (
                <>
