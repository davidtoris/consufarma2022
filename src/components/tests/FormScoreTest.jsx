...
const FormScoreTest = ({ Test, TestAnswer, point }) => {
  ...
  // Determine if Test is an array or object
  const testData = Array.isArray(Test) ? (Test.length > 0 ? Test[0] : null) : Test;
  ...
  const sendEmailTest = async () => {
    if (!testData) return; // Guard against missing data

    const data = {
      testId: TestAnswer.test_id,
      userId: TestAnswer.id,
      correo: TestAnswer.correo,
      nombreCurso: nombre_curso,
      estudiante: TestAnswer.estudiante,
      examenConstancia: 'examen',
      curso_relacionado_uno: testData.curso_relacionado_uno,
      curso_relacionado_dos: testData.curso_relacionado_dos,
      curso_relacionado_tres: testData.curso_relacionado_tres,
    }
  ...
  if (!testData) {
    return <div>No se encontró la información del examen</div>
  }

  const handlePrint = async (UrlScoreDiploma, TipoScoreDiploma) => {
    try {
      const resp = await instanceAPI.get(
        `testsPDF/${UrlScoreDiploma}?testId=${TestAnswer.test_id}&scoreId=${TestAnswer.id}`,
        { responseType: 'blob' }
      );
