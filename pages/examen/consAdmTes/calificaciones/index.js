            ...
            <tr className='border-2 text-1enter' key={t.id}>
              ...
              <td className='border-2 border-gray-200 p-1'>
                <div className='flex text-xl justify-center px-1'>
                  ...
                  {t.score !== 'Sin Calificacion' && (
                    <div onClick={() => copyLink(`${URL_SITE}/examen/resultado/${t.test_id}?student=${t.id}`)} className='cursor-pointer hover:scale-110 transition-all ml-3'><VscCopy title="Copiar Link" className='text-gray-500' /></div>
                  )}
                  {t.score !== 'Sin Calificacion' && (
